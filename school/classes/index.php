...
$classes_list = my_db::Query('select * from classes where active = true order by title');
?>
<div class="content">
    <div class="table">
        <div class="trow thead">
            <div class="col col_n">№</div>
            <div class="col col_title">Название</div>
        ...
        <div class="trow">
            <div class="col col_n"><?=$i?></div>
            <div class="col col_title">Новый</div>
    ...
    <script>
            ...
            target.append(createEl('div', {
                ...
                children: [
                    createEl('input', {
                        ...
                        events: {
                            input: function () {
                                if(this.value.length > 9) this.value = this.value.substr(0, 10);
                            }
                        }
                    })
                ]
                        ...
                        events: {
                            click: function () {
                                ...
                                if(inputTitle.value.length > 1) {
                                    ...
                                    var isFree = syncRequest(PATH+'/ajax.php', {request: 'check_class_title', title: val});
                                    isFree = JSON.parse(isFree);
                                    if(isFree.status == 'free') {
                                        postData(PATH+'/ajax.php', {request: 'add_class', title: val}).then(data => {
        ...
        function changeItem() {
            ...
            optionsBlock.append(createEl('button', {
                ...
                events: {
                    click: function () {
                        ...
                        if(inputTitle.value.length > 1) {
                            var val = inputTitle.value.replaceAll('"', '∞');
                                postData(PATH+'/ajax.php', {request: 'update_item', table: 'classes', id: id, data: {title: '"'+val+'"'}}).then(data => {
                                    var answer = JSON.parse(data);
            ...
            optionsBlock.append(createEl('button', {
                ...
                events: {
                    click: function () {
                        postData(PATH+'/ajax.php', {request: 'delete_class', id: id}).then(data => {
                                console.log(data);
                                var answer = JSON.parse(data);
                                if(answer.status == 'ok') {
